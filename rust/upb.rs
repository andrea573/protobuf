        ...
        $(
            impl<'msg> RepeatedField<'msg, $rs_type> {
                ...
                fn new(arena: &'msg Arena) -> Self {
                    Self { inner: RepeatedFieldInner { raw: unsafe { upb_Array_New(arena.raw, $upb_tag as std::ffi::c_int) }, arena }, _phantom: PhantomData }
                }
                pub fn push(&mut self, val: $rs_type) {
                    unsafe { upb_Array_Append(self.inner.raw, upb_MessageValue { $union_field: val }, self.inner.arena.raw()) }
                }
                pub fn get(&self, i: usize) -> Option<$rs_type> {
                    if i >= self.len() { None } else { unsafe { Some(upb_Array_Get(self.inner.raw, i).$union_field) } }
                }
                pub fn set(&self, i: usize, val: $rs_type) {
                    if i >= self.len() {
                        return;
                    }
                    unsafe { upb_Array_Set(self.inner.raw, i, upb_MessageValue { $union_field: val }) }
                }
            }
        )*
    }
